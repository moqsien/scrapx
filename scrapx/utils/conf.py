...
def closest_scrapy_cfg(path='.', prevpath=None, name='scrapx.cfg'):
    ...
    if os.path.exists(cfgfile):
        return cfgfile
    return closest_scrapy_cfg(path=os.path.dirname(path), prevpath=path, name=name)
...
def init_env(project='default', set_syspath=True):
    ...
    """
    cfg = get_config(name='scrapx.cfg')
    ...
    if closest:
        ...
        projpath = os.path.dirname(projdir)
        if set_syspath and projdir not in sys.path:
            sys.path.append(projdir)
        if projpath not in sys.path:
            sys.path.append(projpath)
    # for workspace
    workspace_cfg = get_config(name='workspace.cfg')
    if workspace_cfg.has_option('settings', 'xglobals'):
        os.environ['SCRAPX_GLOBAL_MODULE'] = workspace_cfg.get('settings', 'xglobals')

    workspace_closest = closest_scrapy_cfg(name='workspace.cfg')
    if workspace_closest:
        workspacedir = os.path.dirname(workspace_closest)

        if set_syspath and workspacedir not in sys.path:
            sys.path.append(workspacedir)


def get_config(use_closest=True, name='scrapx.cfg'):
    """Get Scrapy config file as a ConfigParser"""
    sources = get_sources(use_closest=use_closest, name=name)
    cfg = ConfigParser()
    cfg.read(sources)
    return cfg


def get_sources(use_closest=True, name='scrapx.cfg'):
    xdg_config_home = os.environ.get('XDG_CONFIG_HOME') or os.path.expanduser('~/.config')
    sources = [
        '/etc/{}'.format(name),
        r'c:\scrapy\{}'.format(name),
        xdg_config_home + '/{}'.format(name),
        os.path.expanduser('~/.{}'.format(name)),
    ]
    if use_closest:
        sources.append(closest_scrapy_cfg(name=name))
    return sources
